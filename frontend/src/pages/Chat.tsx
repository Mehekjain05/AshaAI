  ...
  return (
    <div className="max-w-3xl mx-auto">
      {messages.map((message) => (
        ...
        >
          <div
            className={`inline-block max-w-lg py-3 px-4 rounded-2xl break-words shadow-md ${
              message.role === "user"
                ? "bg-[#90c45c] text-white"
  ...
  const handleSubmitMessage = async (e: React.FormEvent) => {
    ...
    if (!conversationToUpdate) {
      ...
      const newConversation: Conversation = {
        id: newConvId,
        title:
          trimmedInput.slice(0, 30) + (trimmedInput.length > 30 ? "..." : ""), // Generate title
        messages: [],
      ...
      console.log(
        "Created new conversation implicitly:",
        newConversation.id,
        "with model:",
    ...
    if (!conversationToUpdate) {
      ...
      setStreamError(
        "An internal error occurred. Please try starting a new chat."
      );
      return;
    }
    ...
    try {
      console.log(
        `Sending request to users/chat for conversation ${conversationId} using model ${modelToUse}`
      );
      ...
      if (!response.ok) {
        ...
        console.error(
          `Failed to start chat stream: ${response.status} ${response.statusText}`,
          errorText
        );
        ...
        try {
          ...
          if (done) {
            ...
            const finalAssistantMessage: Message = {
              ...
              content:
                assistantContent || "Sorry, I couldn't generate a response.", // Use accumulated content or error message
          ...
          for (let part of parts) {
            if (part.startsWith("data: ")) {
              ...
              try {
                ...
                console.error(
                  "Error parsing JSON stream chunk:",
                  e,
                  "Raw data:",
                  jsonStr
                );
      ...
      <div className="px-4 py-3 border-t border-gray-200 bg-white">
        ...
        >
          <input
            ...
            placeholder="Message..."
            className="w-full py-3 pl-4 pr-12 border border-gray-300 rounded-full focus:outline-none focus:ring-2 focus:ring-[#90c45c] focus:border-[#90c45c] transition duration-200 ease-in-out disabled:opacity-50 disabled:bg-gray-100"
            disabled={isStreaming}
            aria-label="Chat message input"
          />
          <button
            type="submit"
            className={`absolute right-2 top-1/2 transform -translate-y-1/2 p-2 rounded-full text-blue-500 hover:bg-blue-100 focus:outline-none focus:ring-2 focus:ring-[#90c45c] transition duration-200 ease-in-out ${
        ...
        <div className="max-w-3xl mx-auto flex justify-between items-center mt-2 text-xs text-gray-500">
          ...
          <div className="flex items-center">
            {isStreaming && (
              <span className="text-gray-500 mr-2 animate-pulse">
                Generating response...
              </span>
            )}
...
const Sidebar: React.FC = () => {
  ...
  const handleNewConversation = () => {
    ...
    console.log(
      "Created new conversation from sidebar:",
      newConversation.id,
      "with model:",
      newConversation.model
    );
    ...
    <div className="flex flex-col h-full bg-gray-50">
      ...
      <div className="p-4 border-b border-gray-200">
        <button
          ...
          className="flex items-center justify-center gap-2 w-full py-2 px-4 border border-[#90c45c] rounded-md bg-white hover:bg-[#e4f7d3] text-[#90c45c] font-semibold transition focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition duration-150 ease-in-out"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            className="h-5 w-5"
      ...
      <div className="px-4 py-3 mb-2 relative border-b border-gray-200">
        <label className="text-xs font-semibold text-[#90c45c] mb-1 block">
          Model
        </label>
        <button
          onClick={() => setIsModelDropdownOpen(!isModelDropdownOpen)}
          className="flex items-center justify-between w-full py-2 px-3 border border-[#90c45c] text-[#90c45c] rounded-md bg-white hover:bg-[#f4fdf0] focus:outline-none focus:ring-2 focus:ring-[#90c45c] transition focus:border-transparent transition duration-150 ease-in-out"
        ...
        >
          ...
          <svg
            xmlns="http://www.w3.org/2000/svg"
            className={`h-4 w-4 text-[#90c45c] transition-transform ${
        ...
        {isModelDropdownOpen && (
          <div
            className="absolute mt-1 w-full bg-white border border-[#90c45c] rounded-md shadow-lg z-10 py-1"
            role="listbox"
          >
            {/* List your available models here */}
            <button
              onClick={() => handleModelSelect("ChatGPT")}
              className="block w-full text-left px-4 py-2 text-sm hover:bg-[#e4f7d3] text-[#90c45c]"
              role="option"
              aria-selected={selectedModel === "ChatGPT"}
            >
              ChatGPT
            </button>
            <button
              onClick={() => handleModelSelect("GPT Turbo")}
              className="block w-full text-left px-4 py-2 text-sm hover:bg-[#e4f7d3] text-[#90c45c]"
              role="option"
              aria-selected={selectedModel === "GPT Turbo"}
            >
              GPT Turbo
            </button>
            <button
              onClick={() => handleModelSelect("Literature Review Writer")}
              className="block w-full text-left px-4 py-2 text-sm hover:bg-[#e4f7d3] text-[#90c45c]"
              role="option"
              aria-selected={selectedModel === "Literature Review Writer"}
            >
      ...
      <div className="flex-1 overflow-y-auto">
        {displayedConversationGroups.map((group, groupIndex) => (
          <div key={group.title + groupIndex} className="mb-2">
            {" "}
            {/* Use title + index for key */}
              ...
              {group.conversations.map((conversation) => (
                <li key={conversation.id}>
                  <button
                    ...
                    className={`flex items-center w-full px-4 py-2.5 text-sm hover:bg-gray-200 focus:outline-none focus:bg-gray-200 transition duration-150 ease-in-out ${
                      currentConversation?.id === conversation.id
                        ? "'bg-[#e4f7d3] text-[#90c45c] font-semibold'"
                        : "hover:bg-[#f7fdf4] text-gray-700"
                    }`}
                  >
        ...
        {displayedConversationGroups.length === 0 && (
          <p className="text-center text-sm text-gray-500 px-4 py-4">
      ...
      <div className="p-4 border-t border-gray-200">
        ...
        <button className="flex items-center gap-2 text-sm w-full p-2 rounded-md hover:bg-[#f0fae8] text-[#90c45c] font-medium transition focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition duration-150 ease-in-out">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            className="h-5 w-5 text-[#90c45c]"
            viewBox="0 0 20 20"
            fill="currentColor"
          >
            <path
          ...
          <div className="text-left">
            ...
            <div className="text-xs">Get more features</div>
          </div>
        </button>
      </div>
    </div>
...
const Header: React.FC<{
  toggleSidebar: () => void;
  currentConversation: Conversation | null;
}> = ({ toggleSidebar, currentConversation }) => {
  // Simple header, displays current conversation title if available
  return (
    <div className="flex items-center px-4 h-14 border-b border-[#90c45c] bg-white flex-shrink-0">
      {/* Sidebar Toggle */}
      <button
        onClick={toggleSidebar}
        className="mr-4 p-1 rounded hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500"
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
      ...
      {currentConversation && (
        <span className="ml-3 text-xs bg-[#90c45c] text-white px-2 py-0.5 rounded-full">
...
const ConversationsProvider: React.FC<{ children: React.ReactNode }> = ({
          ...
          messages: [
            {
              id: "msg-initial-1",
              role: "user",
              content: "How do I set up Python on Windows?",
              timestamp: new Date(),
            },
            {
              id: "msg-initial-2",
              role: "assistant",
              content:
                "To set up Python on Windows, first download the installer from python.org...",
              timestamp: new Date(),
            },
          ],
          model: "ChatGPT",
  ...
  useEffect(() => {
    if (
      ...
      conversations[0].conversations.length > 0
    ) {
...
const Chat: React.FC = () => {
  ...
  return (
    <ConversationsProvider>
      <div className="flex h-screen bg-[#f7fdf4] text-gray-900 font-sans">
        {/* Sidebar */}
        <div
          className={`bg-gray-50 border-r border-gray-200 transition-all duration-300 ease-in-out flex-shrink-0 ${
            isSidebarOpen ? "w-72" : "w-0"
          } overflow-hidden`}
        >
          {isSidebarOpen && <Sidebar />}{" "}
          {/* Conditionally render Sidebar content for clean transition */}
        </div>
